            ...
            for j in range(self.input_branches):
                if i == j:
                    ...
                    self.fuse_layers[-1].append(
                        nn.Sequential(
                            nn.PixelShuffle(upscale_factor=2 ** ((j - i))),
                        )
                    )
                else:
                    # i > j
...
class DeepLabV3PlusFusion(nn.Module):
    def __init__(
        ...
        # ******************** Conv1 ********************
        self.downsample1 = nn.PixelUnshuffle(downscale_factor=2)
    ...
    def forward(self, x):
        # ******************** Conv1 ********************
        x = self.downsample1(x)  # (3, H, W) -> (12, H/2, W/2)
        conv1_features = x  # Conv1层的特征图
        x = self.downsample1(x)  # (48, H/4, W/4)

        # x = self.conv1(x)  # (3,H,W) -> (16,H/2,W/2)
        # x = self.conv2(x)  # (32, H/4, W/4)

        # ******************** Stage1 ********************
        x = self.stage1(x)  # (48, H/4, W/4)
...
def deeplabv3plus_fusion_backbone():
    ...
    stage1_setting = [
        bneck_conf(bc, bc * 4, bc, 3, 1, "RE", False),
        bneck_conf(bc, bc * 4, bc, 3, 1, "RE", False),
    ]
