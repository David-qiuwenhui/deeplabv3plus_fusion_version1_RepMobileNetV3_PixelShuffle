...
class DeepLab(nn.Module):
    def __init__(
        ...
        # 浅层特征的通道数量
        conv1_channels = 12
        stage1_channels = in_channels
        stage2_channels = in_channels
        stage3_channels = in_channels
        stage4_channels = in_channels

        # ASPP模块融合后输出的通道数量
        aspp_channels = in_channels
        ...
        #   第二阶段的深浅层次特征图卷积处理模块
        # ----------------------------------#
        self.pixelupsample = nn.PixelShuffle(upscale_factor=2)
        self.cat_conv2 = nn.Sequential(
            ConvBNActivation(
                in_planes=in_channels // 4 + conv1_channels,
                out_planes=in_channels // 4,
                kernel_size=3,
            ),
            nn.Dropout2d(0.1),
        )
        ...
        self.cls_conv = nn.Conv2d(
            in_channels=in_channels // 4, out_channels=num_classes, kernel_size=1
        )
    ...
    def forward(self, x):
        ...
        #   特征上采样至原图大小
        # -----------------------------------------#
        x = self.cls_conv(x)  # x(bs, N, H/2, W/2)
        x = F.interpolate(
